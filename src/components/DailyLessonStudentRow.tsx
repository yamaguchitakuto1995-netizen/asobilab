...
export function DailyLessonStudentRow({
  ...
  return (
    <>
      <div className={`px-4 py-3 space-y-2 ${isAbsent ? "text-slate-500" : ""}`}>
          ...
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <dl className={`text-[11px] space-y-0.5 ${isAbsent ? "text-slate-500" : "text-slate-600"}`}>
              <div className="flex gap-1">
                <dt className={`shrink-0 ${isAbsent ? "text-slate-400" : "text-slate-400"}`}>学年</dt>
                <dd>{st?.grade ?? "—"}</dd>
              </div>
              <div className="flex gap-1">
            ...
            {slotRow ? (
              <p className={`text-[10px] ${isAbsent ? "text-slate-400" : "text-slate-400"}`}>
