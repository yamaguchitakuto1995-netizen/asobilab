...
import { BackfillMakeupAbsentForm } from "@/components/BackfillMakeupAbsentForm";
...
import { registerMakeupEligibleAbsent } from "../actions";

type Params = Promise<{ id: string }>;
type SearchParams = Promise<{ error?: string }>;

export default async function BackfillAbsentPage({
  params,
  searchParams,
}: {
  params: Params;
  ...
  return (
    <div className="max-w-lg">
      ...
      <PageHeader
        title="欠席（振替可能）を手動登録"
        description={`${student.name} (${student.grade}) — 過去分の振替対象欠席`}
      />
      ...
      <BackfillMakeupAbsentForm
        studentId={student.id}
        cancelHref={`/students/${student.id}`}
        action={registerMakeupEligibleAbsent}
