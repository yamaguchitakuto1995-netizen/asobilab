...
--
-- 1) 振替申請: 授業日の3日前 23:59 (JST) まで
-- 2) 欠席登録: 授業開始時刻 (JST) まで（当日のみ時刻チェック）

-- ===== 共通ヘルパー =====

create or replace function public.assert_makeup_registration_open(p_chain_date date)
returns void
language plpgsql
stable
...
declare
  v_now_jst      timestamp;
  v_deadline_jst timestamp;
  v_deadline_lbl text;
begin
  v_now_jst := now() at time zone 'Asia/Tokyo';
  v_deadline_jst := (p_chain_date - interval '3 days')::date + time '23:59:59';

  if v_now_jst > v_deadline_jst then
    v_deadline_lbl := to_char((p_chain_date - interval '3 days')::date, 'MM/DD');
    raise exception '振替申請は授業日の3日前（% 23:59）までです。', v_deadline_lbl;
  end if;
end;
$assert_makeup$;

revoke all on function public.assert_makeup_registration_open(date) from public;
grant execute on function public.assert_makeup_registration_open(date) to anon, authenticated;

create or replace function public.assert_absence_registration_open(
  p_lesson_date date,
  p_period      smallint,
  p_subject     text,
  p_venue       text
...
begin
  ...
  if p_lesson_date < v_today_jst then
    raise exception 'この授業はすでに終了しているため、欠席登録できません。';
  end if;
  ...
  if v_now_jst >= (p_lesson_date + v_start_time) then
    raise exception '授業開始時刻（%）を過ぎたため、欠席登録できません。', to_char(v_start_time, 'HH24:MI');
  end if;
end;
$assert_absence$;

revoke all on function public.assert_absence_registration_open(date, smallint, text, text) from public;
grant execute on function public.assert_absence_registration_open(date, smallint, text, text) to anon, authenticated;
...
begin
  ...
  if p_lesson_date < v_today_jst
     or p_lesson_date > v_today_jst + interval '120 days' then
