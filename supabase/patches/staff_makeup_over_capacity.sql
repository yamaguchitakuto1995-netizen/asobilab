-- 職員振替登録: 定員超過でも登録可能（保護者申請は従来どおり満員で不可）
-- makeup_availability_from_period_times.sql 適用済みの DB で実行してください。
...
declare
  ...
  v_max_target       date;
begin
  if p_allow_over_capacity and not public.is_staff_user() then
  ...
  if not exists (
    select 1
     ...
     where cpt.classroom = v_venue
       and cpt.lesson_date = p_lesson_date
       and cpt.period = p_period
       and (cpt.subject is null or cpt.subject = p_subject)
  ) then
    raise exception 'この日時のコマ枠は設定されていません。';
  end if;

  select coalesce(lc.max_students, cl.default_max_students, 4)::smallint
    into v_max
    from public.classrooms cl
    left join public.lesson_capacities lc
      on lc.classroom = cl.name
     and lc.day_of_week = extract(dow from p_lesson_date)::smallint
     and lc.period = p_period
     and lc.subject = p_subject
   where cl.name = v_venue;
...
revoke all on function public.book_makeup_lesson(uuid, date, smallint, text, date, smallint, text, text, text, boolean) from public;
grant execute on function public.book_makeup_lesson(uuid, date, smallint, text, date, smallint, text, text, text, boolean) to anon, authenticated;
