-- 振替枠の出し方を「振替枠設定の第◯週」ではなく
-- 「コマ時刻登録（classroom_period_times）の暦日」基準に変更
--
-- 事前に block_past_makeup_target.sql を実行済みであること
-- （assert_makeup_target_bookable が必要な場合）
...
-- book_makeup_lesson: コマ時刻登録がある日のみ予約可（第◯週縛りを廃止）
